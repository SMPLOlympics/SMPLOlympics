...
motion_file = "sample_data/golf_after_1_upright.pkl"
...
def main():
    ...
    for entry_key, data_seq in tqdm(items):
        ...
        with torch.no_grad():
            vertices, joints = smpl_parser.get_joints_verts(pose=torch.from_numpy(pose_aa), th_betas=torch.zeros((1, 10)), th_trans=torch.from_numpy(root_trans_offset), )
