...
class HumanoidBasketballShoot(humanoid_amp_task.HumanoidAMPTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.freethrow_state = joblib.load("sample_data/freethrow_state.pkl")
    ...
    def _compute_statistics(self):
        ...
        if len(self.success) >= self.max_episode_length:
            ...
            print(f"Num Envs: {self.num_envs} Hits: {all_hits/self.num_envs} , Dist-success rate: {success}, Average distance: {average_distance}")
    ...
    def _reset_target(self, env_ids):
        ...
        self._target_states[env_ids, 2] = 1.5
    ...
    def _sample_ref_state(self, env_ids):
        ...
        motion_ids_jv, motion_times_jv, root_pos_jv, root_rot_jv, dof_pos_jv, root_vel_jv, root_ang_vel_jv, dof_vel_jv,  rb_pos_jv, rb_rot_jv, body_vel_jv, body_ang_vel_jv = self.freethrow_state
        ...
        rb_pos[:] = rb_pos_jv
        rb_rot[:] = rb_rot_jv
        root_pos[:, :3] = torch.tensor([self.goal_bound_red[0] - 4.12, 0, 0.93]).to(self.device)
    ...
    def _compute_task_obs(self, env_ids=None):
        ...
        for i in range(self.num_agents):
            ...
            obs = compute_basketball_observations(root_states, tar_states, goal_pos, bounding_box_points,  opponent_root_states, teammate_root_states, ball_spawn_pos)
            obs_list.append(obs)
    ...
    def _compute_reset(self):
        ...
        self.reset_buf[:], self._terminate_buf[:] = compute_humanoid_basketball_reset(
        ...
        if flags.im_eval:
            ...
            not_high_enough  = self._target_states[..., 2] < self._ball_spawn_pos[..., 2] - 0.4
            has_contact = torch.logical_and(has_contact, past_time)
            fail_check = torch.logical_or(has_contact, not_high_enough)
            
            high_enough  = self._target_states[..., 2] > 2
            fail_check[high_enough] = False
            self.reset_buf[:] = torch.logical_or(self.reset_buf, fail_check); self._terminate_buf[:] = torch.logical_or(self._terminate_buf, fail_check)
...
class HumanoidBasketballShootZ(HumanoidBasketballShoot):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
...
@torch.jit.script
def compute_basketball_observations(root_states, tar_states, goal_location, bounding_box_points, opponent_root_states, teammate_root_states, ball_spawn_pos):
...
@torch.jit.script
def compute_desired_velocity_3d(current_position, goal_position):
...
# @torch.jit.script
def compute_basketball_reward(ball_states,  root_state, goal_pos, hand_pos, target_pos_in_goal, prev_ball_pos,  prev_root_pos, dt, ball_spawn_pos, beyond_ball_allowed):
    # type: (Tensor, Tensor, Tensor, Tensor,Tensor,Tensor,Tensor, float, Tensor, float) -> Tuple[Tensor, Tensor]
    pos_err_scale = 5.0
    ...
    # target_pos_reward[ball_goal_velocity <= 0] = 0.0
    
    
    hand_to_ball_pos = (ball_pos[:, None] - hand_pos).norm(dim=-1, p = 2).mean(dim = -1)
    hand_to_ball_closer_r = torch.exp(-10 * hand_to_ball_pos)
    ...
    closer_inxy = (torch.norm((ball_pos -  target_pos_in_goal)[:, :2], dim = -1) - torch.norm((prev_ball_pos - target_pos_in_goal)[:, :2], dim = -1, p = 2)) < -0.01

    reward = torch.zeros_like(hand_to_ball_closer_r)
    passed_ball = ((root_pos[:, 0] - goal_pos[:, 0]) ** 2) < ((ball_spawn_pos[:, 0] - goal_pos[:, 0]) ** 2)
    passed_ball_reward = - (1. - torch.exp(-0.2 * ((root_pos[:, 0] - ball_spawn_pos[:, 0]) ** 2)))
    passed_ball_reward[~passed_ball] = 0.0
    
    desired_ball_vel = compute_desired_velocity_3d(ball_pos, target_pos_in_goal)
    ball_vel_diff = ball_vel - desired_ball_vel
    ball_vel_diff = ((ball_vel_diff)**2).mean(dim=-1)
    r_vel = torch.exp(-0.1 * ball_vel_diff)
    r_vel[~closer_inxy] = 0.0
    hand_to_ball_closer_r[closer_inxy] = 0.0
    ball_goal_velocity_r[~closer_inxy] = 0.0
    
    # max_height = predict_max_height(ball_pos, ball_vel)
    # height_reward = 1. - torch.exp(-max_height)
    
    # print(desired_ball_vel.numpy())
    # reward[:] = ball_player_closer_r[:] * 0.4 + \
    #             ball_to_goal_closer_r[:] * 0.1 + 0.3 * height_reward + 0.1 * ball_goal_velocity_r[:]  + 0.5 * target_pos_reward[:] + passed_ball_reward
    ...
    # reward[:] = 0.1 * hand_to_ball_closer_r + 0.5 * r_vel + 0.5 *  ball_goal_velocity_r + passed_ball_reward
    reward[:] = 0.5 * r_vel + 0.5 *  ball_goal_velocity_r + passed_ball_reward
    reward_raw = torch.stack([hand_to_ball_closer_r, ball_goal_velocity_r, r_vel, passed_ball_reward], dim=-1)
...
def compute_humanoid_basketball_reset(reset_buf, progress_buf, contact_buf_list, contact_body_ids, rigid_body_pos_list,
    ...
    if (enable_early_termination):
        ...
        for i in range(1, num_agents):
            ...
            has_passed_ball = torch.norm(body_xy - green_goal_pos[:, :2].view(-1, 2)) < (torch.norm(ball_spawn_position[:, :2] - green_goal_pos[:, :2].view(-1, 2)) - max_passed_ball)
