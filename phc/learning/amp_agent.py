...
class AMPAgent(common_agent.CommonAgent):
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            update_indices = self.dones.repeat(self.env_num_agents).nonzero(as_tuple=False)
            
            self.game_rewards.update(self.current_rewards[update_indices])
            self.game_lengths.update(self.current_lengths[update_indices])
            self.algo_observer.process_infos(infos, update_indices)
    ...
    def assamble_kin_dict(self, kin_dict_flat):
    ...
    def optimize_kin(self, batch_dict):
        info_dict = {}
        humanoid_env = self.vec_env.env.task
        if humanoid_env.distill:  # VAE distillation
            kin_dict = self.assamble_kin_dict(batch_dict['kin_dict'])
