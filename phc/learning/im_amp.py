...
class IMAmpAgent(amp_agent.AMPAgent):
    def __init__(self, base_name, config):
        ...
        if self.save_kin_info:
            ...
            kin_lr = float(humanoid_env.kin_lr)
            self.kin_dict_info = None
            self.kin_optimizer = torch.optim.Adam(humanoid_env.world_model.parameters(), kin_lr)
        
    def save(self, fn):
        super().save(fn)
        ### Need to fix this later. 
        if self.config.get("learn_world", False):
            if self.epoch_num % min(50, self.save_best_after) == 0:
                print(f'saving world {osp.join(self.network_path, f"world_model.pth")}')
                torch.save(self.vec_env.env.task.world_model.state_dict(), osp.join(self.network_path, f"world_model.pth"))
                
            if (self._save_intermediate) and (self.epoch_num % (self.save_freq) == 0):
                print(f'saving world {osp.join(self.network_path, f"world_model_{self.epoch_num:010d}.pth")}')
                torch.save(self.vec_env.env.task.world_model.state_dict(), osp.join(self.network_path, f"world_model_{self.epoch_num:010d}.pth"))
